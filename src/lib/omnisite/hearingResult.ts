...
export interface HearingResultA {
  ...
  /** 등급 원문(`LOW`/`MEDIUM`/`HIGH`)과 0~1 수용도. 안 온 값은 `null`. */
  metrics: {
    cssPro: string | null;
    cssCon: string | null;
...
/**
 * 이 run 의 토론이 끝났는가 — **임시 판정이다.**
 *
 * 🔴 제대로 된 기준은 「토론 결과가 그 run 으로 **DB 에 저장됐다**」이고, 그건 아직
 *    백엔드에 경로가 없다(저장도 조회도). 그래서 지금은 **이 탭의 sessionStorage 에
 *    기록이 있는가**로 대신 판정한다. 탭을 닫으면 사라지므로 **완료 표시도 사라진다** —
 *    이건 결함이 아니라 지금 우리가 아는 사실의 정확한 반영이다. 서버에 없는 것을
 *    「완료」로 굳혀 두면 다음 사람이 조회되는 줄 안다(원칙 4).
 *
