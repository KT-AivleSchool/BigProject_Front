/**
 * 화면 전체를 **기준 해상도(15" · 1512×982) 캔버스**로 놓고, 실제 화면이 그보다
 * 작을 때만(예: 13") 가로세로 비율을 유지한 채 통째로 줄인다.
 *
 * 2026-08-12, 사용자 지시 — "13인치랑 15인치에서 다르게 보인다" 문제를
 * 컴포넌트별 반응형(브레이크포인트마다 폰트·여백 다시 정하기) 대신 이 방식으로
 * 푼다: 안쪽 내용은 **항상 1512×982 라고 믿고** 그리고, 화면이 그보다 작을 때만
 * `transform: scale()` 로 시각적으로만 축소한다. 그래서
 ...
 *
 * 기준보다 **큰** 화면(15" 이상·외부 모니터)은 배율이 1 에서 안 넘어간다
 * (`min(1, …)`) — 커지지 않고 캔버스 바깥은 `--canvas-soft` 여백으로 남는다.
 * 이 앱이 이미 `max-w-*` 로 큰 화면에서도 무한히 안 늘어나게 잡아놨던 것과
 * 같은 종류의 제한이라 새로운 게 아니다.
 *
    ...
    >
      <div
        style={{
          ...
          transform: `scale(min(1, calc(100dvw / ${baseWidth}px), calc(100dvh / ${baseHeight}px)))`,
