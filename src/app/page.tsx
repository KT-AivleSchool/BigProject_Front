 ...
 *   preset → `fixture` · `hitl`.  둘 다 `datasets/<도메인>_FIX/` 를 요구하고,
    ...
    <PageBody>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 pr-2 mt-4">
        <div className="max-w-5xl mx-auto flex flex-col gap-8 pb-12">
          ...
          {isReadOnly && run && (
            <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-500 shrink-0">
                ...
                <div>
                  ...
                  <span className="sm:ml-2 text-sm text-blue-700 block sm:inline">
                    이전에 실행된 &apos;{run.domain}&apos; 입지 분석 결과를 확인하고 있습니다.
                  </span>
                </div>
              </div>
              <button
          ...
          {/* 🔴 과거 내역에서는 입력 폼을 통째로 지운다(§7-6). 값을 남겨두면
              「이 run 이 이 파일로 돌았다」로 읽히는데, 도메인 폴더의 현재 내용은
              그때와 다를 수 있다 — 없는 정보를 지어내지 않는다. */}
          {isReadOnly ? (
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-12 text-center text-gray-600 shadow-sm">
              ...
              {dataSource === "upload" && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <p className="mb-4 text-sm text-gray-500">
                    조례는 <code className="font-mono text-xs">datasets/&lt;도메인&gt;/law/</code>,
 ...
 *
 * 🔴 「용산구 흡연부스」 같은 **표시명은 API 에 없다.** `DomainItem` 은
 *    `domain`·`law_files`·`data_files`·`has_audit_reviewed` 넷뿐이다(실측
 *    2026-08-10). 그래서 폴더명을 그대로 보여준다 — 예쁜 이름을 프런트에
 *    적어두면 도메인이 늘 때마다 화면이 거짓말한다.
 */
  ...
  return (
    <>
      ...
      <p className="mt-5 text-xs leading-relaxed text-gray-500">
        ...
        <code className="font-mono">datasets/&lt;도메인&gt;_FIX/</code>)가 있어야 합니다.</b> 없으면
