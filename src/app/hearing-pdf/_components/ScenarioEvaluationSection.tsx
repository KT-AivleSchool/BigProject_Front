...
export function ScenarioEvaluationSection({ scenarios }: ScenarioEvaluationSectionProps) {
  ...
  return (
    <section className="mb-8">
      <h2 className="text-base font-bold text-slate-900 mb-3 flex items-center gap-2 border-l-4 border-blue-600 pl-3">
        <FileText size={18} className="text-blue-600" />
        2. 시나리오 심의 평가 및 종합 의견
      </h2>
      ...
      {scenarios.map((sc, idx) => (
        <div key={idx} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden mb-4">
          <div className="bg-slate-900 text-white px-5 py-3 flex justify-between items-center text-xs">
            <span className="font-bold text-sm">시나리오 {sc.scenario}: {sc.scenario_description}</span>
          ...
          <div className="p-5 space-y-4 text-xs leading-relaxed">
            <div>
              <h3 className="font-bold text-slate-700 mb-1 text-xs">심의 종합 요약</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white p-3.5 rounded-lg border border-slate-200">
                <h4 className="font-bold text-slate-500 mb-1">협상/수용도 평가 사유</h4>
            ...
            {sc.used_doc_ids && sc.used_doc_ids.length > 0 && (
              <div className="text-slate-400 text-[11px]">
                참조 관련 조례/문서 ID: {sc.used_doc_ids.join(", ")}
              </div>
            )}
          </div>
        </div>
