 ...
 *
 * `saved` 는 **결과가 서버에 남는지**다. 지금 A 만 남는다(`conflict_simulations`
 * + `debate_logs`, 조회는 `GET /api/v1/simulations/results/{parcel_id}`).
 * B 는 저장·조회 경로가 아예 없어서(`/stakeholders` 는 `generate`·`discuss/stream`
 * 둘뿐이다 — 2026-08-11 `openapi.json` 실측) **이 브라우저 탭에만** 남는다.
 * 고르기 전에 알아야 하는 차이라 카드에 적는다. 백엔드에 저장 경로가 생기면
 * 이 문구를 **같이** 고쳐야 한다.
 */
const MODES = [
  {
  ...
  },
  {
    ...
    saved: "결과가 서버에 저장되지 않습니다 — 이 탭에만 남습니다.",
  },
];
